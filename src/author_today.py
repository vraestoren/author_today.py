...
class AuthorToday:
    ...
    def get_login_cookie_by_token(self, token: str) -> dict:
        return self._get(
            f"{self.web_api}/account/login-cookie-by-token?token={token}")
    ...
    def get_work_details(
        ...
        return self._get(f"{self.api}/v1/work/{work_id}/details?recommendationsCount={recommendations_count}")
    ...
    def search(self, title: str) -> dict:
        return self._get(f"{self.web_api}/search?q={title}")
    ...
    def get_home_page(self, view: str = "all") -> dict:
        return self._get(f"{self.api}/v1/home/home-page?View={view}")
